...
export default function Dashboard() {
    ...
    const [isCategoryHistoryMode, setIsCategoryHistoryMode] = useState(false);
    ...
    const [catDrillDown, setCatDrillDown] = useState({ show: false, category: '', items: [] });
    const [dayDrillDown, setDayDrillDown] = useState({ show: false, dateStr: '', items: [] });

    const chartCategoriasRef = useRef(null);
    const chartEvolucaoRef = useRef(null);
    const chartCategoryEvolRef = useRef(null);
    ...
    const kpi = useMemo(() => {
        ...
        currentMonthData.forEach(t => t.valor > 0 ? receitas += t.valor : despesas += Math.abs(t.valor));
    ...
    const handleMonthChange = (e) => {
        ...
        setCatDrillDown({ show: false });
        setDayDrillDown({ show: false });
    };
    ...
    useEffect(() => {
        ...
        // PALETA CREPÚSCULO COM TRANSPARÊNCIA (65% de opacidade)
        const baseColors = [
            "rgba(31, 166, 122, 0.65)",  // Verde Porto
            "rgba(242, 183, 5, 0.65)",   // Amarelo Farol
            "rgba(242, 116, 5, 0.65)",   // Laranja Pôr do Sol
            "rgba(9, 130, 140, 0.65)",   // Teal Profundo
            "rgba(130, 91, 140, 0.65)",  // Roxo Crepúsculo
            "rgba(74, 118, 148, 0.65)",  // Azul Aço
            "rgba(84, 140, 91, 0.65)",   // Verde Musgo
            "rgba(191, 101, 115, 0.65)", // Rosa Velho/Coral
            "rgba(191, 156, 65, 0.65)",  // Mostarda Areia
            "rgba(103, 163, 179, 0.65)"  // Azul Gelo
        ];
        // Cores 100% sólidas para o efeito de "acender" ao passar o rato
        ...
        chartInstances.current.cat = new Chart(chartCategoriasRef.current, {
            ...
            data: { 
                ...
                datasets: [{ 
                    ...
                    borderWidth: 0,               // Remove as bordas duras
                    hoverOffset: 40               // Afasta bastante a fatia (Efeito interativo)
                }] 
            },
            options: {
                responsive: true, 
                maintainAspectRatio: false, 
                cutout: "60%",                    // Deixa a 'rosquinha' mais fina e elegante
                layout: {
                    padding: 10                   // Espaço extra para a fatia não cortar quando expandir
                },
                plugins: { 
    ...
    useEffect(() => {
        ...
        chartInstances.current.evol = new Chart(chartEvolucaoRef.current, {
            ...
            options: {
                ...
                plugins: { tooltip: { mode: 'index', intersect: false, callbacks: { label: (ctx) => ` ${ctx.dataset.label}: ${formatCurrency(ctx.raw)}` } } },
        ...
        if (isCategoryHistoryMode) {
            ...
            transactions.forEach(t => {
                ...
                if (mesesParaExibir.includes(transMes)) {
                    const parts = (t.split && t.split.length > 0) ? t.split : [t];
                    parts.forEach(s => catSet.add(s.categoria || "Não classificada"));
                }
            });
        } else {
            currentMonthData.forEach(t => {
...
useEffect(() => {
        // 1. ESCUDO EXTRA: Se o usuário desmarcou tudo, destrói o gráfico
    ...
    function getUltimos6Meses(mesBaseIso) {
        const arr = [];
    ...
    const toggleCategoriaDropdown = (e, cat) => {
        e.preventDefault();
        e.stopPropagation(); // Impede que o clique feche o menu do Bootstrap
        setUserSelectedCategories(prev =>
            prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat]
        );
    };

    const abrirDetalhesCategoria = (categoria) => {
        const itens = [];
        currentMonthData.forEach(t => {
            if (t.valor >= 0) return;
        ...
        itens.sort((a, b) => a.valorReal - b.valorReal);
        setCatDrillDown({ show: true, category: categoria, items: itens });
    };

    const abrirDetalhesDia = (dia) => {
        const [ano, mes] = selectedMonth.split('-');
        const dataStr = `${String(dia).padStart(2, "0")}/${String(mes).padStart(2, "0")}/${ano}`;
        const itens = currentMonthData.filter(t => t.data === dataStr && t.valor < 0).sort((a, b) => a.valor - b.valor);
    ...
    const renderHeatmapGrid = () => {
        ...
        for (let d = 1; d <= diasNoMes; d++) {
            ...
            if (val > 0) {
                ...
                // Aqui o style inline é OBRIGATÓRIO, pois a cor (hue) muda dia após dia baseada na matemática!
                cells.push(
                            ...
                            {catDrillDown.show && (
                                <div className="w-100 h-100 d-flex flex-column absolute-top drilldown-overlay radius-bottom-16">
                                    <div className="d-flex justify-content-between align-items-center px-4 py-2 border-bottom border-secondary border-opacity-25">
                                        ...
                                        <button className="btn btn-sm btn-outline-light" onClick={() => setCatDrillDown({ show: false })}>
                            ...
                            {dayDrillDown.show && (
                                <div className="w-100 h-100 d-flex flex-column position-absolute top-0 start-0 drilldown-overlay radius-bottom-16">
                                    <div className="d-flex justify-content-between align-items-center px-4 py-2 border-bottom border-secondary border-opacity-25 mt-2">
                                        ...
                                        <button className="btn btn-sm btn-outline-light" onClick={() => setDayDrillDown({ show: false })}>
                                            <i className="bi bi-arrow-left"></i> Voltar
                                        </button>
                                    </div>
                                    <div className="list-group list-group-flush flex-grow-1 overflow-auto bg-transparent">
            ...
            <div className="theme-surface shadow-sm mb-4">
                <div className="card-header bg-transparent border-bottom border-secondary border-opacity-25 py-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div className="d-flex align-items-center gap-3">
                        ...
                        {/* DROPDOWN CONTROLADO PELO REACT */}
                        <div className="dropdown position-relative" ref={filterRef}>
                            ...
                            {/* A lista só é renderizada se isFilterOpen for true */}
                            {isFilterOpen && (
